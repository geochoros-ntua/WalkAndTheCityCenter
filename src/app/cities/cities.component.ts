...
export class CitiesComponent implements OnInit {
  ...
  getDataRecordsArrayFromCSVFile(csvRecordsArray: any, headerLength: any) {
    ...
    for (let i = 1; i < csvRecordsArray.length; i++) {
      ...
      if (curruntRecord.length === headerLength) {
        ...
        // cityItem.selected_variable = Number(curruntRecord[3].trim());
  ...
  sortCities(variable: any) {
    ...
    this.cities.sort((a: any, b: any) => {
      return b[variable.value.value] - a[variable.value.value];
    });

    for (let index = 0; index < this.cities.length; index++) {
      this.cities[index].order_number = index + 1;
      this.cities[index].selected_variable = this.cities[index][variable.value.value];
    }
